...
resource "linode_instance" "accounting_app" {
  ...
  metadata  {
  user_data = base64encode(templatefile("${path.module}/install_wordpress.tftpl", {label="accounting-app",wppw=onepassword_item.accounting_db.password,gcagg=var.gcagg_ip,gcuium=data.onepassword_item.gcuium.password,mysqlip=local.accounting_db_ip,wordpressname="accounting-app.${var.domain}", proxy=local.proxy_ip}))
  }

...
parser = argparse.ArgumentParser(description='approve connector in EAA')
...
for connector in result.json()['objects']:
    if connector['name'] == args.name[0]:
        newresult=s.post(urljoin(baseurl, '/crux/v1/mgmt-pop/agents/%s/approve' % connector['uuid_url']))
        print(newresult)

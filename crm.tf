...
resource "null_resource" "eaa_crm" {
  depends_on = [linode_instance.crm_web]
  
  provisioner "local-exec" {
      command = "${path.module}/publish_app.py crm ${var.domain} ${local.crm_web_ip} ${data.external.eaa_connector.result.agentid}"
  }
  provisioner "local-exec" {
    when    = destroy
    command = "${path.module}/unpublish_app.py crm"
  }  
}
